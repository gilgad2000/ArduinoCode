...
#define DEVICE_ADDRESS 0x0A  //  Decimal 10
...
{ 
  int i;
  /**  Set up output pins.  **/
  pins[0]=LED0; pins[1] = LED1; pins[2]=LED2;
...
void loop()
{
   ...
   /**if((CAN.readStatus()&0x04)==0x04)
   {
     CAN.readMsgBuf0(&(inst.len), inst.receiveBuf);
     CAN.clearRX0Status();
     processBuffer();
   }
   if((CAN.readStatus()&0x08)==0x08)
   {
     //**  Once new message arrives retrieve it.  **
     CAN.readMsgBuf1(&(inst.len), inst.receiveBuf);
     //**  Reset interrupts.  **
     CAN.clearRX1Status();
     processBuffer();
   }*/
